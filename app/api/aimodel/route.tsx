...
export async function POST(req: NextRequest) {
  ...
  // Rate limit enforcement disabled — allow unlimited generation.
  // Previously returned a JSON error when `decision.isDenied()` and rate-limited.
  ...
  try {
    ...
    } else if ((result as any).response && (result as any).response.output) {
      text = (result as any).response.output;
    } else if (Array.isArray((result as any).candidates) && (result as any).candidates[0]) {
      text = (result as any).candidates[0].content ?? (result as any).candidates[0].output ?? JSON.stringify((result as any).candidates[0]);
    } else {
